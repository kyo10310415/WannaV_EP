...
| `MAX_VIDEO_UPLOAD_MB` | `2048`（動画1ファイルの上限。必要に応じてMB単位で変更） |

> 大容量動画を保存する場合は、Render の **Disks** 画面で、既存動画を含む合計容量より十分に大きい Disk サイズを確保してください。アプリ上限を増やしても Disk の空き容量が不足しているとアップロードは完了しません。
