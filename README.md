...
2. **レッスン管理**
   - 動画アップロード（最大500MB）
   - レッスンの順序制御
   - 順次解禁コースでは、それ以前の全レッスンの完了（小テストがあれば合格）まで後続をロック。途中への追加・並べ替えでも既存の完了記録は保持

3. **進捗管理**
   - ユーザーごとの進捗追跡
   - 完了率の可視化
   - 生徒のアプリ利用回数を日次・月次・生徒別に集計
   - 小テスト全問正解で次へ進む
...
- **ダッシュボード** (`/dashboard`) - ユーザー情報と進捗表示
  - 管理者が期間と任意のURLを設定できる[重要メッセージ](docs/important-message.md)を表示
  - キャラクター未選択の生徒は、具体的なレッスン開始日（未設定時はNotionの開始月）が2026年10月以降、または開始日未設定の場合に選択可能。2026年9月以前は選択ボタンを表示しない。既存の選択結果は表示する。
...
Portal Visit・学習KPI・支払い同期の定義と段階導入手順は
[運用ガイド](docs/portal-kpi-payment.md) を参照してください。
...
6. **app_usage_daily** - 生徒の日別アプリ利用回数
7. **notifications** - 通知ログ
